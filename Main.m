...
%chose the task
task = 'interpretation'% 'interpretation','predeiction'
...
elseif strcmp(dataset_name,'FHZ')
 ...
 elseif strcmp(dataset_name,'Covid19_newcases')
    ...
    data =TT{2:end,46:end};
    data5=[]
    data5=[data5; cellfun(@str2num, data(:,1:end))]
    %data5=data5([1:39,41:end],6:end);
    if strcmp(task,'interpretation')
        File_name = 'Covid_new_case_model_K_30_K_prime16_interpretation'
        portion = 0.920;
        Model_hyper_params.alpha1 = 1;
        Model_hyper_params.beta1 = 0.08;%1e-2%*ones(K,1);
    elseif strcmp(task,'predeiction')
        File_name = 'Stock_model_K_30_K_prime16_predictionb_0001'
        portion = 0.920;
        Model_hyper_params.alpha1 = 1;
...
if ~ strcmp(dataset_name,'Covid19_newcases')
    ...
    if strcmp(task,'predeiction')
        ...
        [XP_temp,abser1,XP_temp2,abser2] = Prediction_Ksteps_count(X,X_w,T_P,X_dim_S,S0_col,S_col,Phi_prime_col,K,W_col,Z_col,D_col,X_W_avg,T,rr_col);
...
%         %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
        ...
        TrX = cat(2, TRdata, TEdata);
        for i = 1:size(X_W_avg(1:end,1:end),1)
            figure()
            ...
            % fill(x2, inBetween, '--g');
            hold on;
            plot([1:84], MyX(i,1:end), 'b');

            temp22 = permute(squeeze(XP_temp2(i,:,:,:)),[1 3 2]);
            temp22 = reshape(temp22,[],size(temp22,3),1);
            xxx=[78:84];
            shadedErrorBar(xxx,temp22,{@mean,@(xxx) 2*std(xxx)},'lineProps',{'y','markerfacecolor','y'})
            % std_temp11 = std(temp11);
            % curve11 = MyX(i,78:end) + 2*std_temp11;
            % curve12 = MyX(i,78:end) - 2*std_temp11;
            % x2 = [[78:84], fliplr([78:84])];
            ...
            % fill(x2, inBetween, '--g');
            hold on;
            plot([1:84], MyX2(i,1:end), 'b');
            hold on
            plot([1:84],TrX(i,1:end),'r')
            hold on 
            xline(77,'--k');
            hold off
            title(sprintf('observation dimension %d ',i));
